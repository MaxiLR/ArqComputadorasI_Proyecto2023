...
http_return GetMessage(const char *botID, int offset);
void SendMessage(const char *botID, char *chat_id, const char *message);

void string_buffer_initialize(string_buffer_t *sb) {
  sb->len = 0;
  sb->ptr =
      malloc(sb->len + 1); /* will be grown as needed by the realloc belove */
...
size_t string_buffer_callback(void *buf, size_t size, size_t nmemb,
                              void *data) {
...
http_return http(char *mymethod, char *myurl, char *mydata) {
  ...
  http_return HTTP_RESULT = {.update_id = 0, .text = ""};
...
http_return GetMessage(const char *botID, int offset) {
  ...
  snprintf(url, sizeof(url),
           "https://api.telegram.org/bot%s/getUpdates?offset=%d", botID,
           offset);
...
void SendMessage(const char *botID, char *chat_id, const char *message) {
  ...
  int url_size =
      snprintf(NULL, 0, "https://api.telegram.org/bot%s/sendMessage", botID);

  // Asignar memoria dinámica para la URL
  char *url = malloc(url_size + 1);
  snprintf(url, url_size + 1, "https://api.telegram.org/bot%s/sendMessage",
           botID);
...
//   //......bot_token...../sendMessage?chat_id=....&text=Hola%20Luis";
//   string_buffer_initialize(&strbuf);
